...
export function TeacherProfile({ teacher, data, calculatePerformance, onClose, onSelectQuiz }: TeacherProfileProps) {
  const teacherSubjects = useMemo(() => data.subjects.filter(s => 
    (s.teacherId === teacher.id || s.teacherIds?.includes(teacher.id)) && !s.isArchived
  ), [data.subjects, teacher.id]);

  const teacherQuizzes = useMemo(() => data.quizzes.filter(q => {
  ...
  const teacherVisits = useMemo(() => data.visits.filter(v => v.teacherId === teacher.id && !v.isArchived), [data.visits, teacher.id]);
        ...
        <div className="bg-white p-8 pb-6 flex items-center gap-6 relative">
           ...
           <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3 mt-3">
                 <span className="px-4 py-1.5 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest">{teacher.specialization || 'عضو هيئة التدريس'}</span>
                 <span className="px-4 py-1.5 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black border border-slate-200">الرمز: #{teacher.id.slice(-6).toUpperCase()}</span>
                 <button 
           ...
           <div className="w-full md:w-80 bg-slate-50 p-6 border-l border-slate-100 flex flex-col gap-8 overflow-y-auto">
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1 mb-6 flex items-center gap-2">
                  <TrendingUp size={14} className="text-indigo-500" /> تحليل مؤشرات الأداء
                </p>
                
                <div className="h-56 w-full mb-6 bg-white rounded-[24px] p-4 shadow-sm border border-slate-100">
                   <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats.classStats.slice(0, 8)} margin={{ top: 15, right: 0, left: -25, bottom: 0 }}>
                         <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" opacity={0.5} />
                         <XAxis dataKey="shortName" tick={{ fill: '#64748b', fontSize: 9, fontWeight: 700 }} axisLine={false} tickLine={false} />
                         ...
                         />
                         <Bar name="الأداء" dataKey="perf" fill="#4f46e5" radius={[4, 4, 0, 0]} maxBarSize={20}>
                            <LabelList dataKey="perf" position="top" fill="#4338ca" style={{ fontSize: '8px', fontWeight: 'bold' }} />
                            {stats.classStats.map((entry, index) => (
                              <Cell key={`cell-${index}`} fill={entry.perf >= 85 ? '#10b981' : entry.perf >= 70 ? '#4f46e5' : '#ef4444'} />
                            ))}
                         </Bar>
                      </BarChart>
                   </ResponsiveContainer>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center text-center">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">المواد</p>
                    <p className="text-xl font-black text-indigo-600">{teacherSubjects.length}</p>
                  </div>
                  <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center text-center">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">الزيارات</p>
                    <p className="text-xl font-black text-blue-600">{teacherVisits.length}</p>
                  </div>
                </div>

                <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm">
                   <div className="flex items-center justify-between mb-4">
                      <p className="text-[10px] font-black text-slate-800">المؤشر العام</p>
                      <span className="px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-lg text-[9px] font-black border border-emerald-100">{stats.avg}%</span>
                   </div>
                   ...
                   {stats.avg >= 85 && (
                     <div className="mt-4 flex items-center gap-2 p-2 bg-yellow-50 rounded-xl border border-yellow-100">
                       ...
                       <p className="text-[9px] font-black text-yellow-700">أداء تعليمي متميز</p>
                     </div>
                   )}
                </div>
              </div>
              ...
              <div className="mt-auto pointer-events-none">
                 <div className="flex items-center gap-3 opacity-30 justify-center">
                    ...
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.3em]">نظام إتقان الأساسي</p>
                    <div className="w-1 h-1 rounded-full bg-slate-300" />
                 </div>
              </div>
           </div>
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    {teacherSubjects.map(sub => {
                       ...
                       return (
                         <div key={sub.id} className="bg-slate-50/50 p-5 rounded-[28px] border border-slate-100 flex flex-col gap-4">
                            <div className="flex justify-between items-start">
                               <div>
                                  <p className="font-black text-slate-800 text-sm leading-tight mb-1">{sub.name}</p>
                                  <p className="text-[10px] text-slate-400 font-bold">{grade?.name || 'مرحلة غير محددة'}</p>
                               </div>
                            ...
                            <div className="space-y-2">
                               {affectedClasses.map(cls => {
                                 const perf = calculatePerformance(cls.id, sub.id);
                                 return (
                                   <div key={cls.id} className="flex items-center justify-between p-2.5 bg-white rounded-xl border border-slate-100/80">
                                      <div className="flex items-center gap-2">
                                         <div className="w-1 h-1 rounded-full bg-indigo-400" />
                                         <span className="text-[11px] font-black text-slate-600">{cls.name}</span>
                                      </div>
                                      <span className={`text-[11px] font-black ${perf >= 85 ? 'text-emerald-600' : perf >= 70 ? 'text-indigo-600' : 'text-rose-600'}`}>
                                        {perf}%
                                      </span>
                                   </div>
                                 );
                         ...
                         {teacherQuizzes.length > 0 ? teacherQuizzes.map(quiz => (
                           ...
                           >
                              <div className="flex items-center gap-3">
                                 ...
                                 <div>
                                    <p className="text-xs font-black text-slate-800 group-hover:text-amber-600 transition-colors truncate max-w-[140px]">{quiz.title}</p>
                                    <p className="text-[9px] text-slate-400 font-bold">{quiz.subjectName}</p>
                                 </div>
                              </div>
                              <ChevronLeft size={16} className="text-slate-200 group-hover:text-amber-500 group-hover:-translate-x-1 transition-all" />
                           </button>
                         )) : (
                           <div className="p-10 bg-slate-50 rounded-3xl border border-dashed border-slate-200 text-center">
                              <BrainCircuit size={24} className="mx-auto text-slate-300 mb-2" />
                              <p className="text-[10px] font-black text-slate-400">لا يوجد اختبارات</p>
                           </div>
                         )}
                      </div>
                   </section>
                   ...
                   <section>
                      ...
                      <div className="space-y-3">
                         {teacherVisits.length > 0 ? teacherVisits.map(visit => {
                            ...
                            const totalS = Object.values(evaluationData).reduce((a: number, b: number) => a + b, 0);
                            const totalM = Object.keys(evaluationData).length * 4;
                            const perc = totalM > 0 ? Math.round(((totalS as number) / totalM) * 100) : 0;
                            
                            return (
                              <div key={visit.id} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center justify-between group hover:border-blue-200 transition-all">
                                 <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-1.5xl bg-blue-50 text-blue-600 flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all">
                                       <Calendar size={18} />
                                    </div>
                                    <div>
                                       <p className="text-xs font-black text-slate-800 group-hover:text-blue-600 transition-colors truncate max-w-[140px]">{visit.supervisorName || 'زيارة إشرافية'}</p>
                                       <p className="text-[9px] text-slate-400 font-bold">{new Date(visit.date).toLocaleDateString('ar-SA')}</p>
                                    </div>
                                 </div>
                                 <div className={`px-2.5 py-1 rounded-lg text-[9px] font-black shadow-sm ${perc >= 85 ? 'bg-emerald-500 text-white' : 'bg-slate-50 text-slate-400 border border-slate-100'}`}>
                                    {perc}%
                                 </div>
                              </div>
                            );
                           ...
                           <div className="p-10 bg-slate-50 rounded-3xl border border-dashed border-slate-200 text-center">
                              ...
                              <p className="text-[10px] font-black text-slate-400">لا يوجد زيارات مسجلة</p>
                           </div>
                         )}
                      </div>
                   </section>
