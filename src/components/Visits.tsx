...
export function Visits({
  ...
  // Peer / Mutual Visits State
  const [visitTypeTab, setVisitTypeTab] = useState<"supervisory" | "peer">(
    "supervisory",
  );
  const [visitType, setVisitType] = useState<"supervisory" | "peer">(
    "supervisory",
  );
  const [visitorTeacherId, setVisitorTeacherId] = useState("");
  const [visitTerm, setVisitTerm] = useState<"term1" | "term2">(activeTerm);
  ...
  const handleSave = async () => {
    ...
    if (visitType === "peer" && !visitorTeacherId) {
      alert("يجب اختيار المعلم الزائر في الزيارة التبادلية");
      return;
    }
    ...
    try {
      ...
      alert(
        "حدث خطأ أثناء حفظ الزيارة. يرجى التحقق من اتصالك بالإنترنت والبريد الإلكتروني.",
      );
    }
  };
  ...
  const resetForm = () => {
    ...
    setQuizUrl("");
    setEvaluationData({});
    setVisitType(visitTypeTab);
    setVisitorTeacherId("");
  };

  const handleEdit = (visit: Visit) => {
    setEditingId(visit.id);
    setVisitTerm(visit.term || activeTerm);
    ...
    setQuizType(
      visit.quizId ? "app_quiz" : visit.quizUrl ? "external_url" : "text",
    );
    setQuizId(visit.quizId || "");
    setQuizUrl(visit.quizUrl || "");
    setEvaluationData(visit.evaluationData || {});
    setVisitType(visit.visitType || "supervisory");
    setVisitorTeacherId(visit.visitorTeacherId || "");
    setIsAdding(true);
  };
  ...
  return (
    <div className="w-full h-full overflow-y-auto p-4 md:p-6 pb-32">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-5 rounded-3xl shadow-sm border border-slate-100 gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-slate-500">
                {visitTypeTab === "peer"
                  ? "سجل وتتبع وتقارير الزيارات التبادلية والتعاونية بين المعلمين لتبادل الخبرات والتطوير المهني"
                  : "سجل وتتبع وتقييم المعلمين مع اختيار طلاب لتقييمهم"}
              </p>
            </div>
          </div>
          ...
          {!isAdding && (
            ...
            >
              <Plus size={18} />{" "}
              {visitTypeTab === "peer"
                ? "تسجيل زيارة تبادلية جديدة"
                : "تسجيل زيارة إشرافية جديدة"}
            </button>
          )}
        </div>
        ...
        {/* Tab Navigation */}
        {!isAdding && (
          <div className="flex bg-slate-100/85 p-1 rounded-2xl border border-slate-200/60 max-w-md">
            <button
              onClick={() => {
                setVisitTypeTab("supervisory");
                setVisitType("supervisory");
              }}
              className={`flex-1 py-2 px-5 rounded-xl text-xs font-black flex items-center justify-center gap-2 transition-all ${visitTypeTab === "supervisory" ? "bg-indigo-600 text-white shadow-md" : "text-slate-600 hover:text-slate-800"}`}
            >
              <Focus size={15} />
              الزيارات الإشرافية
            </button>
            <button
              onClick={() => {
                setVisitTypeTab("peer");
                setVisitType("peer");
              }}
              className={`flex-1 py-2 px-5 rounded-xl text-xs font-black flex items-center justify-center gap-2 transition-all ${visitTypeTab === "peer" ? "bg-indigo-600 text-white shadow-md" : "text-slate-600 hover:text-slate-800"}`}
            >
          ...
          <div className="bg-white p-6 md:p-8 rounded-[32px] shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-5 w-full">
              <div>
                ...
                <select
                  value={visitTerm}
                  onChange={(e) => setVisitTerm(e.target.value as "term1" | "term2")}
                  className="w-full text-xs font-black bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 outline-none focus:ring-2 focus:ring-indigo-500"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setTargetClassId("");
                    setSelectedStudentIds([]);
                    // If same as visiting teacher, reset visiting teacher
                    if (
                      visitType === "peer" &&
                      e.target.value === visitorTeacherId
                    ) {
                      setVisitorTeacherId("");
                      setSupervisorName("");
                    }
                ...
                >
                  ...
                  {data.teachers
                    .filter(
                      (t) =>
                        !t.isArchived &&
                        (visitType !== "peer" || t.id !== visitorTeacherId),
                    )
                    .map((t) => (
              ...
              <div>
                {visitType === "peer" ? (
                  <>
                    ...
                    <select
                      value={visitorTeacherId}
                      onChange={(e) => {
                        const selectedId = e.target.value;
                        setVisitorTeacherId(selectedId);
                        const tObj = data.teachers.find(
                          (t) => t.id === selectedId,
                        );
                        setSupervisorName(tObj?.name || "");
                      }}
                      className="w-full text-xs bg-slate-50 border border-slate-200 rounded-md px-2.5 py-1.5 font-bold outline-none focus:ring-2 focus:ring-indigo-500"
                    >
                      <option value="">اختر المعلم الزائر...</option>
                      {data.teachers
                        .filter(
                          (t) => !t.isArchived && t.id !== targetTeacherId,
                        )
                        .map((t) => (
                          <option key={t.id} value={t.id}>
                            {t.name}
                        ...
                        .filter(
                          (s) =>
                            ...
                            (!targetTeacherId ||
                              ...
                              s.teacherIds?.includes(targetTeacherId)),
                        )
                        .map((s) => [s.name, s])
                    ).values()
                  ).map((s) => (
              ...
              <div>
                ...
                >
                  ...
                  {data.rubrics
                    ?.filter((r) => !r.isArchived && (visitType === "peer" ? r.isPeerRubric : !r.isPeerRubric))
                    .map((r) => (
                    ...
                    .filter((c) => {
                      ...
                      data.subjects.forEach((s) => {
                        if (
                          ...
                          if (s.classIds)
                            s.classIds.forEach((id) => teacherClasses.add(id));
            ...
            {targetClassId && (
              <div className="mb-6 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                <p className="font-black text-slate-700 mb-4">
                  اختيار الطلاب للاختبار القصير
                </p>
                <div className="flex flex-wrap gap-2 mb-6">
                  {data.students
                    .filter((s) => !s.isArchived && s.classId === targetClassId)
                    .map((s) => {
                ...
                {selectedStudentIds.length > 0 && (
                  <div className="space-y-4 pt-4 border-t border-slate-200 cursor-default">
                    <div className="flex gap-4 mb-4">
                      ...
                      <label className="flex items-center gap-2 text-sm font-bold text-slate-700 cursor-pointer">
                        <input
                          type="radio"
                          name="quizType"
                          checked={quizType === "app_quiz"}
                          onChange={() => setQuizType("app_quiz")}
                          className="accent-indigo-600"
                        />
                        اختبار من المنصة
                      </label>
                      <label className="flex items-center gap-2 text-sm font-bold text-slate-700 cursor-pointer">
                        <input
                          type="radio"
                          name="quizType"
                          checked={quizType === "external_url"}
                          onChange={() => setQuizType("external_url")}
                          className="accent-indigo-600"
                        />
                        رابط إلكتروني سريع
                      </label>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            value={quizMaxScore}
                            onChange={(e) =>
                              setQuizMaxScore(Number(e.target.value))
                            }
                            className="w-full text-xs bg-white border border-slate-200 rounded-md px-2.5 py-1.5 font-bold outline-none focus:ring-2 focus:ring-indigo-500"
                          />
                        </div>
                    ...
                    {quizType === "external_url" && (
                      <div>
                        <label className="block text-xs font-bold text-slate-500 mb-1">
                          رابط الاختبار الإلكتروني (Google Forms, Microsoft
                          Forms...)
                        </label>
                        <input
                          type="url"
                    ...
                    <div className="space-y-2 mt-4">
                      ...
                      {selectedStudentIds.map((studentId) => {
                        ...
                        return (
                          <div
                            key={studentId}
                            className="flex items-center gap-4 bg-white px-4 py-2 rounded-xl border border-slate-100 shadow-sm"
                          >
            ...
            <div className="flex gap-3">
              ...
              >
                <Save size={18} />{" "}
                {editingId ? "حفظ التعديلات" : "اعتماد الزيارة"}
              </button>
              <button
                onClick={() => {
                  setIsAdding(false);
        ...
        {(() => {
          const filteredVisits = (data.visits || []).filter(
            (v) => {
              ...
              if (filterTeacherId) {
                return v.teacherId === filterTeacherId || v.visitorTeacherId === filterTeacherId;
              }
              return true;
            }
          );
          ...
          if (filteredVisits.length === 0) {
            const isPeer = visitTypeTab === 'peer';
            return (
              <div className="relative p-12 rounded-[3rem] text-center shadow-sm max-w-lg mx-auto overflow-hidden border transition-all duration-300 bg-white border-slate-100">
                <div className="absolute inset-0 bg-gradient-to-br from-[#faf9f6]/30 to-transparent pointer-events-none" />
                
                <div className={`w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6 border shadow-inner transition-transform duration-500 hover:scale-[1.05] ${
                  isPeer 
                    ? 'bg-amber-500/10 text-amber-600 border-amber-200/50' 
                    : 'bg-indigo-600/10 text-indigo-600 border-indigo-200/50'
                }`}>
                  {isPeer ? <ArrowLeftRight size={34} className="animate-pulse" /> : <Focus size={34} className="animate-pulse" />}
                </div>
                
                <h3 className="font-extrabold text-slate-800 mb-2 text-base">
                  {isPeer ? 'سجل الزيارات التبادلية فارغ' : 'سجل الزيارات الإشرافية فارغ'}
                </h3>
                
                <p className="text-xs leading-relaxed text-slate-500 font-medium mb-6 max-w-md mx-auto">
                  {isPeer 
                    ? 'لا غنى عن تبادل الزيارات بين الزملاء لتبادل استراتيجيات التعليم ونشر المعرفة. لم يتم رصد أي زيارات متبادلة حتى اللحظة لهذا الفصل الدراسي.' 
                    : 'الزيارات الإشرافية تسهم في توجيه الكفاءات التعليمية وتتبع الخطط الدراسية. يرجى البدء بإنشاء زيارة للرصد.'}
                </p>

                <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[10.5px] font-black border uppercase tracking-wider ${
                  isPeer 
                    ? 'bg-amber-500/5 text-amber-700 border-amber-500/20' 
                    : 'bg-indigo-600/5 text-indigo-700 border-indigo-600/20'
                }`}>
                  <span className="w-1.5 h-1.5 rounded-full animate-ping bg-current" />
                  <span>{isPeer ? 'جاهز للتسجيل والمشاركة المهنية' : 'بانتظار رصد الزيارة الأولى للمشرف'}</span>
                </div>
              </div>
            );
          }
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {filteredVisits.map((visit) => {
                const teacher = data.teachers.find(
                  (t) => t.id === visit.teacherId,
                );
                const students = data.students.filter((s) =>
                  visit.selectedStudentIds?.includes(s.id),
                );
                ...
                return (
                  <div
                    ...
                    className="bg-white rounded-[24px] shadow-sm border border-slate-250 flex flex-col justify-between hover:shadow-md hover:border-indigo-100 transition-all overflow-hidden group cursor-pointer"
                  >
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-4 gap-2">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-10 h-10 rounded-[12px] bg-slate-50 flex items-center justify-center text-slate-400 group-hover:text-white transition-all shadow-inner ${visit.visitType === "peer" ? "group-hover:bg-amber-500" : "group-hover:bg-indigo-600"}`}
                          >
                            {visit.visitType === "peer" ? (
                          ...
                          <div>
                            <h3 className="font-black text-sm md:text-base text-slate-800 leading-tight group-hover:text-indigo-600 transition-colors">
                              {teacher?.name || "معلم غير معروف"}
                            </h3>
                            <p className="text-[10px] font-black text-slate-400 flex items-center gap-1 mt-1">
                        ...
                        <div
                          className={`px-2.5 py-1 rounded-full text-[9px] font-black shadow-sm shrink-0 ${
                            totalPercentage >= 90
                      ...
                      <div className="flex flex-col gap-2">
                        <div className="flex flex-wrap gap-2">
                          {visit.supervisorName && (
                            <div
                              className={`${visit.visitType === "peer" ? "bg-amber-50/80 text-amber-700" : "bg-indigo-50 text-indigo-700"} text-[10px] font-black px-2.5 py-1 rounded-md max-w-full`}
                            >
                              <span className="truncate">
                                {visit.visitType === "peer"
                                  ? `الزائر: ${visit.supervisorName}`
                                  : `المشرف: ${visit.supervisorName}`}
                              </span>
                            </div>
                          )}
                          {visit.rubricId && (
                            <div className="bg-slate-50 text-slate-600 text-[10px] font-black px-2.5 py-1 rounded-md max-w-full border border-slate-100">
                              <span className="truncate">
                                {data.rubrics?.find(
                                  (r) => r.id === visit.rubricId,
                                )?.name || "غير معروف"}
                              </span>
                            </div>
                          )}
                    ...
                    <div className="p-6 md:p-8">
                      <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h2 className="font-black text-xl text-slate-800 leading-tight">
                              {teacher?.name || "معلم غير معروف"}
                            </h2>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                          ...
                          <div
                            className={`text-lg font-black ${totalPercentage >= 90 ? "text-emerald-500" : totalPercentage >= 75 ? "text-sky-500" : totalPercentage >= 50 ? "text-amber-500" : "text-rose-500"}`}
                          >
                            {totalPercentage}%
                          </div>
                        </div>
                        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                          ...
                          <div className="text-sm font-black text-slate-700 truncate">
                            {data.subjects.find((s) => s.id === visit.subjectId)
                              ?.name || "غير محدد"}
                          </div>
                        </div>
                        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                      ...
                      {visit.evaluationData &&
                        Object.keys(visit.evaluationData).length > 0 && (
                          <div className="mb-6 space-y-3">
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                              {data.rubrics
                                ?.find((r) => r.id === visit.rubricId)
                                ?.categories.map((category) => {
                                  let catScore = 0;
                                  let catMax = 0;
                                  category.items.forEach((item) => {
                                    if (visit.evaluationData?.[item.id]) {
                                      catScore += visit.evaluationData[item.id];
                                      catMax += 4;
                                    }
                                  });
                                  ...
                                  return (
                                    ...
                                    >
                                      <span className="text-xs font-black text-slate-700 flex-1 truncate">
                                        {category.title}
                                      </span>
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span
                                          className={`text-[10px] font-black w-8 text-left ${percentage >= 80 ? "text-emerald-600" : percentage >= 60 ? "text-amber-600" : "text-red-600"}`}
                                        >
                                          {percentage}%
                                        </span>
                                      </div>
                      ...
                      {(visit.quizUrl ||
                        ...
                        <div className="bg-slate-50 rounded-2xl p-4 sm:p-5 border border-slate-100">
                          <p className="text-[11px] font-black text-slate-500 mb-4 flex items-center gap-1.5">
                            ...
                            الطلاب المستهدفين ({students.length}) والتقييم
                            السريع
                          </p>
                            ...
                            <div className="bg-white rounded-xl p-3 mb-4 shadow-sm border border-slate-100">
                              ...
                              {visit.quizUrl ? (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="text-sm font-black text-indigo-600 hover:underline break-all"
                                >
                                  {visit.quizUrl}
                                </a>
                              ) : visit.quizId ? (
                            ...
                            <div className="flex flex-col gap-2">
                              {students.map((s) => {
                                ...
                                return (
                                  <div
                                    key={s.id}
                                    className="bg-white flex items-center justify-between px-4 py-2.5 rounded-xl border border-slate-100 shadow-sm"
                                  >
                                    <div className="flex items-center gap-2">
                                      <div
                                        className={`w-2 h-2 rounded-full ${score !== undefined ? "bg-emerald-500" : "bg-slate-300"}`}
                                      />
